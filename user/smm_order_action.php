...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        // --- NAYA BADLAAO: Comments ko DB mein save karein ---
        $stmt_order = $db->prepare("
            INSERT INTO smm_orders (user_id, service_id, link, quantity, charge, comments, status, provider_order_id)
            VALUES (?, ?, ?, ?, ?, ?, 'pending', NULL)
        ");
        // Comments ko $comments variable se pass karein
        $stmt_order->execute([$user_id, $service_id, $link, $quantity, $charge, $comments]);
